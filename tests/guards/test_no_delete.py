"""The store appends. Its interface has no way to remove a key.
Kubernetes may reclaim a verified terminal Job.
...
design rests on, and the cheapest way to keep it is to have no store `delete`: an interface with
...
DELETE_CALL = re.compile(r"\.delete\(|[\"\']DELETE[\"\']")
...
def test_nothing_calls_delete_on_a_store() -> None:
    offenders = [
        ...
        and not (
            path == REPO_ROOT / "packages/dispatch/src/redteam_dispatch/k8s.py"
            and line.strip().startswith('"DELETE",')
        )
    ]
