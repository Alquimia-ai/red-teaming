...
SURFACE = ("packages/target/src", "packages/engine/src", "packages/dispatch/src")
"""Where a status is read off a response: the target's answers, and the platform's -- a daemon's
conflict, an API server's not-found -- which the dispatcher spells by name too."""
